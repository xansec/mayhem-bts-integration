...
```
usage: bts_export.py [-h] --workspace WORKSPACE --project PROJECT --target TARGET --bts BTS [--defect DEFECT] [--run RUN] [--severity SEVERITY] [--output-csv] [--bts-config BTS_CONFIG] [--mayhem-config MAYHEM_CONFIG] [--use-pass]
                     [--log LOG] [--insecure] [--dry-run]
...
optional arguments:
  ...
  --severity SEVERITY   Severity level to export (i.e. "high"; defaults to all defects)
  --output-csv          Output results in CSV format instead
